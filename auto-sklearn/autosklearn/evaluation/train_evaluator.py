...
from autosklearn.evaluation.curriculum_generator import InstanceHardness as IH
...
def subsample_indices(train_indices, subsample, task_type, Y_train):
    ...
    if subsample is not None and subsample < 1:
        ...
        curriculum_learning = IH().switch_curriculum()

        if curriculum_learning:
            #Load complete curriculum
            curriculum = IH().load_curriculum()

            #Select from curiculum examples that were chosen by autosklearn for training
            curriculum = curriculum.iloc[train_indices]

            anti_curriculum = IH().anti_curriculum()
            
            if anti_curriculum:
            #Sort data from hard to easy -> Anti-Curriculum
                curriculum_indices = curriculum.sort_values(by=['score'], ascending=False)
            else:
            #Sort data from easy to hard -> Curriculum
                curriculum_indices = curriculum.sort_values(by=['score'])              

            #Train indices for subsample are now following the curriculum order.
            train_indices = curriculum_indices["index"].values
